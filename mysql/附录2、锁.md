...
<font color="red">It is also worth noting here that conflicting locks can be held on a gap by different transactions. For example, transaction A can hold a shared gap lock (gap S-lock) on a gap while transaction B holds an exclusive gap lock (gap X-lock) on the same gap. The reason conflicting gap locks are allowed is that if a record is purged from an index, the gap locks held on the record by different transactions must be merged.</font>
...
```mysql
RECORD LOCKS space id 47 page no 4 n bits 72 index idx_code of table `demo`.`t_gap_lock_demo` trx id 13077 lock_mode X locks gap before rec insert intention waiting
```
...
<font color="red">If a duplicate-key error occurs, a shared lock on the duplicate index record is set.</font>

<font color="red">先加间隙锁后加插入意向锁会被阻塞，先加插入意向锁后加间隙锁不会阻塞？？？</font>

<font color="red">xsh商品中心死锁场景二，可能存在间隙锁？？？</font>
