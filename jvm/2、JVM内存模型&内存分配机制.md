...
如果Minor GC后，老年代没有足够的内存空间存放年轻代需要移动到老年代的对象时，会触发Full GC；Full GC后，JVM还是会做一次Minor GC，如果老年代还是没有足够的内存空间存放年轻代需要移动到老年代的对象，会发生"OOM"；如果GC后，还是没有足够的内存空间给新对象分配内存，也会发生"OOM"。
